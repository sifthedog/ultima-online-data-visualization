...
RSpec.describe SkillAttempt do
  ...
  it "stores skills and outcomes under the shard's own names" do
    ...
    expect(described_class.skills.values).to contain_exactly(
      "Blacksmithy", "Bowcraft/Fletching", "Mysticism", "Magery", "Tailoring", "Inscription", "Tinkering", "Mining", "Alchemy", "Carpentry", "Lumberjacking", "Hiding", "Spellweaving"
    )
    expect(described_class.outcomes.values).to contain_exactly("made", "failed", "cast", "fizzled", "dug", "smelted", "chopped", "converted", "hidden")
